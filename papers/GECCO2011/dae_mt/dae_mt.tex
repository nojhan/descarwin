...
\category{I.2.8}{Computing Methodologies}{Artificial In\-tel\-ligen\-ce}[Problem Solving, Control Methods, and Search]
%
\category{D.1.3}{Pro\-gramming Techniques}{Concurrent Programming}[Parallel Programming]

% A category with the (minimum) three required fields
%\category{H.4}{Information Systems Applications}{Miscellaneous}[Problem Solving, Control Methods, and Search]
...
simple {\it run level parallelism}, EA with {\it global parallelism}
where the evaluation stage is parallelized but not the other operators, cellular EAs
(cEA) where the selection and reproduction steps are parallelized, and the {\it dEA}
approach, using a multi-population {\it island parallelism} \cite{alba:IEEE2002}.

The run level parallelism being trivially achieved by running several runs concurrently, the first step when parallelizing a new algorithm is often to try the global
parallelization approach, because the evaluation step is often the most costly part of EAs and because individual evaluations are generally intrinsically independent.

%Doing a review of existing algorithms being beyond the scope of this paper, we redirect the reader on the review of Alba and Tomassini \cite{alba:IEEE2002}.
...
memory limitation through a retraction mechanism which allow a processor to
free its memory by dropping states. In order to confirm the transfer of a state,
synchronous communication channels must be used, which seriously slow down
...
dovetailing, in which several instances of a search algorithm with different parameter
settings are run in parallel. Finally, \cite{vidal:socs2010} proposed a multi-core version of the
...
Standing for Evolving Objects, EO is a largely used and known framework in evolutionary computation based on the C++ template approach. It helps at designing evolutionary algorithms for hard optimization problems, from continuous to combinatorial ones.

A parallel extension of EO, \PARADISEO, has been proposed by Cahon et al. \cite{paradiseo:JHeuristics2004}. It is based on the Message Passing Interface and proposes several parallelization schemes, available by calling wrappers around common functions of EO. \PARADISEO\ targets multi-processors machines, especially clusters and grids, for running large-scale, general purpose EAs.
As we are targeting a multicore architecture, and since our evaluation function has a large memory footprint, message passing may introduce a time consuming memory copy overhead. 
This reason discards the \PARADISEO\ framework.

For a multicore architecture, a shared-memory parallelization approach is thus a better candidate. 
In order to garantee a non-blocking algorithm, we apply the Concurrent Read Exclusive Write (CREW) strategy\footnote{Multiple processors can read a memory cell but only one can write at a time.} of the Parallel Random Access Machine (PRAM).
In this work, we have used \OPENMP\ which defines a set of compiler directives to tell which part of a program should be parallelized and which part of memory can be shared out. 
...
this is the function {\tt apply}. The {\tt apply} function takes a functor as argument.
The advantages of the {\tt Functor} pattern is to get the genericity and the modularity given by
...
\paragraph{\DAEYAHSP\ Specific Parallelization Issues} % (JD)
...
must be guaranteed reentrant and not use static variables nor attributes. A way
...
% FIXME: in restart mode an instance launched with more than 1 thread stops
% before the 30 minutes defined with --max-seconds, Pierre things that the
% reason is because the time is defined in user-time and not in wall-clock.
% FIXME: .out empty, why ?!?
...
\begin{figure}[htpb]
 ...
 \end{center}
 \caption{Wallclock time distributions of 11 runs of \DAEYAHSP\ reletive to the number of processors used.}
...
\paragraph{Speed-up against population size} % (CC, JD)
...
multicore machine increases linearly with the size of the population. The
dispersion of the distribution is due to the steady-state stopping criterion
...
and that it decrease with the population size. The measures were done according
