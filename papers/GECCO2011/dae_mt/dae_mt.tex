...
This preliminary work opens an avenue for parallelizing any evolutionary algorithm developed with EO.
...
instanciated from a set of predicates and a set of objects.
Instances of the planning problem, called planing tasks, can model all kinds of abstract reasoning problems and are known to be NP-hard.

To solve such planning tasks, numerous heuristic search algorithms have been proposed in the past but none of them are easily parallelizable.
They require a great amount of technical work to provide efficiency while preserving correctness \cite{burns:ijcai2009,burns:JAIR2010}.
...
As an evolutionary algorithm \DAEX\ provides an inherent parallelism: the individual evaluation stage, which is the most time-consuming stage during the evolutionary generation loop.
...
Moreover, the implementation of \DAEX\ has been made with the STL-based Evolving Objects framework\footnote{\url{http://eodev.sf.net}} which provides an abstract control structure to develop any kind of evolutionary algorithm. Therefore our parallelization scheme is easily transposable to any evolutionary algorithm developed with the EO framework.


% Il manque un argument pour dire que les archis parallel shared-memory sont bien adaptées à notre cas au contraire des GPGPU (on a pas choisi une archi multi-core au hasard !)
For the proof of concept we implemented a multi-threaded version of \DAEYAHSP, the instantiation of \DAEX\ with the heurictic forward search \YAHSP\ planner \cite{yahsp:icaps2004}, using the OpenMP directive-based API\footnote{\url{http://www.openmp.org}}. 
The design of experiment is built on problems extracted from the last international planning competition\footnote{\url{http://ipc.icaps-conference.org}} with the multithreaded \DAEX\ release mapped onto a 48-core parallel machine.

% FIXME : plan de l'article

\section{Previous Works in Parallel EA and Parallel Planning}

\paragraph{Parallel EA}

A large litterature deals with the parallelization techniques of evolutionary
algorithms. Depending on the targeted architecture, the parallelization scheme
may be chosen differently. The main approches may be classified among: simple
{\it independant runs parallelism}, EA with {\it global parallelism} where the
evaluation is decentralized but not the other operators, cellular EAs (cEA)
where the selection and reproduction steps are decentralized, {\it dEA} approach
using a multi-population {\it island parallelism}. 

The runs parallelism being trivially achieved by running several runs, the first
step when parallelizing a new algorithm is often the lobal parallelization
approach, which is prefered because the evaluation step is often the most costly
part of EAs and because it is intrinsiquely race-conditions free.
...
several parallelization scheme, available by calling wrappers around common
...
We have choose not to use this framework because we were targeting multicore
architecture, where message passing is somewhat useless and may introduce an
overhead higher than other approaches. For multicore architecture, shared
memory parallelization approach is a better candidate as it does not necessitate
heavy librairies linked to the core program. In this work, we have used
OpenMP\footnote{OpenMP: \url{http://www.openmp.org}}, that use a set of compiler
directives to tell the compiler which part of a program should be parallelized.
The main advantage of OpenMP is that it is designed to make easy to parallelize
an existing program.
...
(accelarating) elevators that stop only in floors that are multiple of M/2 (so M
...
Y<X).  There are costs associated with each elavator starting/stoping and
...
inherently parallel, as it use independant variation operators.
...
% Do you agree below?
A further research axis deals with the impact on solution quality and in particular how to take advantage of the parallelization scheme to escape from premature convergence.
