...
Results show that the speedup raises linearly with the number of processors, for every version and every problem tested (see figure \ref{fig:exp2_speedup_core_openstack_17_pop_96_nosm-static_vs_sm-dynamic} for an example on two versions).
...
This observation meets the expected behavior and is classical when considering the parallelization of the evaluation step of an evolutionary algorithm. Nevertheless, the difference among versions and problems can be seen when considering efficiency.
...
\caption{Makespans of the 4 versions of \DAEYAHSP\ on \OPENSTACKS-17.}
\label{fig:exp2_makespan_openstacks-17_pop_96}\end{center}
\end{figure}
