...
there exists a synchronization step at each iteration, this ensure that no race-condition
...
A first observation on Figures \ref{fig:exp2_efficiency_core_elevators_12_pop_96} and \ref{fig:exp2_efficiency_core_openstacks_17_pop_96} is that the speedup is linear with the number of processors. 
For \ELEVATORS-12, there is no much difference between static versus dynamic and between shared and local memorization. 
On \OPENSTACKS-17, the dynamic scheme performs slightly better. The hypothesis was that since the evaluation times are very much dissimilar by construction a queue-based scheduling should be more efficient. But as the latest experience highlights, the algorithm converges prematurely so that most of the individuals are identical, and thus produce equal evaluation times which favors the static scheme. 
...
An explanation for the poor performance of shared memorization might be that the benefit obtained when memorization is shared among individuals is lost with the bottleneck due to multiple access locking.

The dispersion of the distributions is due to the steady-state stopping criterion that may be reached after different times.

%show that on both problems, the computation time decreases with the number of processors used. Comparing to the
%sequential version (running on a single processor), the parallel one permit a decrease that is similar on both problems. 
...
The idea that a larger population would lead to a greater exploration and thus to a better solution quality was not confirmed by experimentation. It means that the algorithm converges very rapidly, independently of the population size. 
The efficiency drop with a population over 1000 individuals might be explained by an increasing of time spent on the variation stage of the algorithm which has not been yet parallelized.
...
\end{center}
\caption{Efficiency of \DAEYAHSP\ relative to the population size, on \OPENSTACKS-17}
...
\end{center}
\caption{Total costs for increasing population sizes, on \ELEVATORS-12}
...
\end{center}
\caption{Makespans for increasing population sizes, on \OPENSTACKS-17}
