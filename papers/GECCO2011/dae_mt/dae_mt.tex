...
The speedup as well as the efficiency are measured with the operating system {\tt time} command which gives the percentage of the CPU that the process being timed got\footnote{computed as (Total number of CPU-seconds that the process spent in user mode + Total number of CPU-seconds that the process spent in kernel mode) / Wall clock time.}.
For instance 3182\% means a speedup of 31.82.
The efficiency is the use rate of the whole processors available. It is computed in dividing the speedup by the number of processors used.
For instance $\frac{31.82}{48} = 66.3\%$.
...
Results show that the speedup raises linearly with the number of processors, for
every version and every problem tested (see figure
...
average.   But  the  behaviour   is   dependent   of   the   algorithm instance
...
schemes may not  be  immediatly notable.
...
probability to reach better solutions. Results show that this does not permit
...
Figure \ref{fig:exp1_efficiency_pop} shows that the efficiency decreases with the
size of the population, while being close to the optimum for a size equal to the
available number of processors, it rapidly decreases and reaches a plateau for
larger populations.
...
framework, this scheme is immediately available for any  evolutionary  algorithms
