...
This experiment uses the same version of \DAEYAHSP\ than the one  used  for  the
...
has 30 minutes to achieve its search, and performs a complete restart each  time
it is stucked during 50 generations without improvement.  Number of  cores  used
range   from   6   to   48,   11   runs   being   performed   each   6    cores.


\paragraph{Alternative version, varying population size}

This experiments test performances of four versions of the algorithm,  combining
...
each  parameter  setting,  which  are  stopped  when  the  algorithm  reach   50
