...
% have to describe in this part the 4 experiments done, here are the summaries of these experiments:
...
% we use as problems: elevator12 et openstack17
% we are using the restart mode
% the stop criteria is the time limit fixed to 30"
% we repeat tests 11 times
% and the used parallelization scheme is the static one

% EXP2
% we are using 48 and 96 individuals as population size
% through the core' sizes: 6, 12, 18, 24, 30, 36, 42, 48
% we are testing on dae and dae-sm (shared memoization)
% we use as problems: openstack17 and elevator12
% we dont use the restart mode
% no stopping criteria defined
% we repeat tests 21 times
% and the used parallelization schemes are the both static and dynamic ones
...
\begin{figure}[htpb]
 ...
 \end{center}
 \caption{Efficiency distributions of 11 runs of \DAEYAHSP\ relative to the population size, on the both sequential \ELEVATORS-12 and temporal \OPENSTACKS-17 problems, using 48 processors.}
