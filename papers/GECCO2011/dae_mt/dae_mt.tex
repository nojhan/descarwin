...
% overhead is not really the good word I think maybe we have to replace it 'waiting'
...
% is the paragraph below still truth ?
In \DAEX, the running time of
...
several threads, when such a subroutine or some subroutine called it modifies
...
% the text below doesnot seem to be right
...
Two types of experiments were conducted to analyse the algorithm behaviour when
...
time it is stucked after 50 generations without improvement. 11 runs
...
\begin{center}
  ...
  \caption{Speedup of \DAEYAHSP\ relative to the number of processors, on
  \OPENSTACKS-17, on the left version using separated memoization and a static
  parallelization scheme, on the right the version using shared-memoization and
  the dynamic parallelization scheme.}
\label{fig:exp2_speedup_core_openstack_17_pop_96_nosm-static_vs_sm-dynamic}
\end{center}
\end{figure}

Results shows that the speedup raise linearly with the number of processors, for
every versions and every problem tested (see figure
\ref{fig:exp2_speedup_core_openstack_17_pop_96_nosm-static_vs_sm-dynamic} for an
example on two versions).

This observation meets the expected behaviour and is classical when  considering
the  parellelization  of  the  evaluation  step  of  an  evolutinary  algorithm.
Nevertheless,  the  difference  among  versions  and  problems   can   be   seen
...
\end{figure}
 ...
 A first observation on figures
\ref{fig:exp2_efficiency_core_elevators_12_pop_96} and
\ref{fig:exp2_efficiency_core_openstacks_17_pop_96}  shows that for the benchmark
\ELEVATORS-12, shared memoization may performs better for larger population  size
but that there is  no  difference  between  static  an  dynamic  parallelization
schemes.  On  \OPENSTACKS-17,  the  dynamic  scheme  performs  slightly  better
when used along with the shared memoization.

Since the evaluation times are varying (given the fact that  decompostions  have
different length and that each subproblem may be more  or  less  difficult),  by
construction  a  queue-based   scheduling   should   be   more   efficient   (in
average).  But the expected behaviour is very dependent of the instance  and  of
the  ability  of  the  algorithm  to  avoid  premature  convergence  that  would
produce an  homogeneous  population.   Hence  the  difference  between  the  two
scheme may not  be  immediatly notable.

An explanation for the small performance gain of using shared memoization  might
...
\caption{Total costs of the 4 versions of \DAEYAHSP\ on \ELEVATORS-12}
\label{fig:exp2_totalcost_elevators-12_pop_96}\end{center}
\end{figure}

Figure \ref{fig:exp2_totalcost_elevators-12_pop_96} shows that on  \ELEVATORS-12
the total cost remains unaffected by the  number  of  cores  used  (results  are
similar on the other problem).

It wàs expected that the decreasing time needed to  perform  a  run  would  lead
to more restarts within the 30  minutes  contract  and  thus  to  an  increasing
probability to reach better solutions. Results shows that this does not permits
significant improvement.  Again, an explanation for those results  is  that  the
...
\paragraph{Speedup against Population Size}
...
\caption{Efficiency of \DAEYAHSP\ relative to the population size, on \ELEVATORS-12}
\label{fig:exp1_efficiency_pop}
\end{figure}

Figure \ref{fig:exp1_efficiency_pop} shows that the effiency decrease with the
size of the population, while being close to the optimum for a size equal to the
available number of processors, it rapidly decrease and reach a plateau for
larger populations.

Using a static parallelization scheme, the decrease in speedup  is  due  to  the
...
%
%The
%efficiency drop with a population over 1000 individuals might be explained by an
%increasing of time spent on the variation stage of the algorithm which  has  not
%been yet parallelized.
%
...
\begin{figure}[htpb]
\begin{center}
\subfigure[\ELEVATORS-12]{\includegraphics[width=0.23\textwidth]{images/EXP1_Total_cost_IPC6_SEQ_ELEVATORS_12_RESTART_dae-sm_STATIC_.pdf}}
\subfigure[\OPENSTACKS-17]{\includegraphics[width=0.23\textwidth]{images/EXP1_Makespan_IPC6_TEMPO_OPENSTACKS_17_RESTART_dae-sm_STATIC_.pdf}}
\end{center}
...
solution qualities can be observed when the population size increase.

The hypothesis that a larger population would lead to a greater exploration  and
thus to a better solution quality is thus not confirmed by experimentation.   It
means that the algorithm converges very rapidly, independently of the population
size. 
...
% we have to change according to the new results
As expected, on our 48-core Dell  Machine  a  roughly  $\times45$  speed-up  was
obtained on  the  tested  benchmark,  this  speed-up  being  the  same  for  all
instances.
...
dispersion and the hardness of  the  problem  solved  remains  to  be  explored.
...
% Have to be changed regarding the new results
The  dynamic  queue  mapping  management  of  threads  onto  cores   provide   a
significant improvement on a single  problem  instance,  when  combined  with  a
shared memoization. 
...
% Dans l'etat pour aller plus vite il faut utiliser une petite taille de population
% car on obtient un meilleur speedup et une meilleur qualité.

% TO ADD IN FURTHER WORK
% to parallelize variation operators
% travailler sur convergence prématurée
% to compare shared/dynamic sheme
% to hybrid shared and distributed parallelization ?!?
