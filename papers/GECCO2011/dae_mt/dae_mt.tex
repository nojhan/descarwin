...
In order to parallelize a large scope of the framework EO, an analyze was done to identify the most used resources by almost all structures. The most EO classes use the pattern ``Functor''. They can be describe as follow: take a population in input and make their own cooking. This is done by iterating through the population.\\

Hence the population's manipulation would be our target to parallelize. Indeed, since we have several steps in evolutionary algorithms, in a most cases the operations done on individuals can be made in parallel without data dependancies. At this time the only operation parallelized is the evaluation. There are several classes through a long hierachy in EO implementing evaluation operators but all of them calls a common function to apply the evaluation, this is the function ``apply''.

\paragraph{EO::apply} % (CC)

The function called ``apply''\footnote{Look at the header file apply.h available in the sources of EO} takes in parameter a population and an operation to apply at each individual.\\
