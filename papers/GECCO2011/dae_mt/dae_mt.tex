...
Experiments show significant speedups with increasing number of cores.
...
computing, multi-core architectures are gaining popularity as they become the
...
part of EAs and because evaluation are generally intrinsically independent.

Doing a review of existing algorithms being beyond the scope of this paper, we 
redirect the reader on the review of Alba and Tomassini \cite{alba:IEEE2002}.
...
Standing for Evolving Object\footnote{Evolving Objects:
...
Passing Interface (MPI\footnote{with MPICH2:
...
We have chosen not to use this framework because we were targeting a multicore
...
In order to parallelize a large scope of the framework EO, an analysis was done
to identify the most used resources. Most of the EO
...
generally take a population (a vector of decompositions) as argument.
...
CREW\footnote{CREW: Concurrent Read Exclusive Write} model.
...
In order to enable OpenMP to build a multi-threaded algorithm without side
effects, the evaluation functor (the ``proc'' in the apply function) must
...
size)-ES: $n$ individuals generate $7 \times n$ offspring without selection.
...
OpenMP let the choice between static and dynamic task scheduling. The default one is the static scheduling.
In the static scheduling, a loop iterating through a set of tasks will be
divided in several tasks according to the number of available processors.
...
\begin{figure}[htpb]
 ...
 \end{center}
 \caption{Absolute speedup distributions of 11 runs of \DAEYAHSP\ according to the population size, on the both sequential elevators-12 and temporal openstacks-17 problems, using 48 processors.}
