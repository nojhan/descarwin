...
For the proof of concept we implemented a multi-threaded version of \DAEYAHSP, the instantiation of \DAEX\ with the heuristic forward search \YAHSP\ planner \cite{yahsp:icaps2004}, using the OpenMP directive-based API\footnote{\url{http://www.openmp.org}}. 
...
step when parallelizing a new algorithm is often the global parallelization
approach, which is preferred because the evaluation step is often the most costly
part of EAs and because it is intrinsically race-conditions free.
...
The parallelization of this region of code is done thanks to the pragma ``omp for''. This can be translated in the PRAM model by the use of CREW\footnote{CREW: Concurrent Read Exclusive Write} model.
