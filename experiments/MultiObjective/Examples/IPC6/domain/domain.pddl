...
(:functions (total-cost)
            ...
            (travel-slow-time ?f1 - count ?f2 - count)
            (travel-fast-time ?f1 - count ?f2 - count) 
)

(:action move-up-slow
  :parameters (?lift - slow-cost-elevator ?f1 - count ?f2 - count )
  :duration (= ?duration (travel-slow-time ?f1 ?f2))
...
(:action move-down-slow
  :parameters (?lift - slow-cost-elevator ?f1 - count ?f2 - count )
  :duration (= ?duration (travel-slow-time ?f2 ?f1))
...
(:action move-up-fast
  :parameters (?lift - fast-cost-elevator ?f1 - count ?f2 - count )
  :duration (= ?duration (travel-fast-time ?f1 ?f2))
  :precondition (and (lift-at ?lift ?f1) (above ?f1 ?f2 ) (reachable-floor ?lift ?f2) )
  :effect (and (lift-at ?lift ?f2) (not (lift-at ?lift ?f1)) (increase (total-cost) (travel-fast-cost ?f1 ?f2))))

(:action move-down-fast
  :parameters (?lift - fast-cost-elevator ?f1 - count ?f2 - count )
  :duration (= ?duration (travel-fast-time ?f2 ?f1))
  :precondition (and (lift-at ?lift ?f1) (above ?f2 ?f1 ) (reachable-floor ?lift ?f2) )
