...
namespace daex {
  ...
  /*
template< class EOT >
EOT rng_choice( EOT container, unsigned int N )
{
    if( N > container.size() ) {
        std::ostringstream msg;
        msg << "N = " << N << " but must be < " << container.size();
        throw std::range_error( msg.str() );
    }

    std::vector< unsigned int > candidates_index;
    ...
    for( unsigned int i=0; i < N; i++ ) {
        unsigned int r = rng.random( container.size() );
        result.push_back( container[r] );
        candidates_index.erase( candidates_index.begin() + r );
    }
...
// Construit un goal intermédiaire dans ]t1,t2]
