...
//    eo::log << eo::logging << FORMAT_LEFT_FILL_W_PARAM << "debug_param" << debug_param << std::endl;
 ...
 // MODIFS MS END
    ...
    // parse les pddl
    // FIXME ATTENTION : il y a une option cach�e dans l'init qui pr�cise � yahsp si on est en temporel ou en s�quentiel, il faut la r�gler correctement en fonction du probl�me vis�
#ifndef NDEBUG
    ...
    // Note : dans le init, l_max est r�gl� au double du nombre de dates dans la partition
...
//#endif // SINGLE_EVAL_ITER_DUMP
 ...
 |____________________________________________|
    ...
    */
    ...
    // MODIFS MS START 
    // pour plus d'output (recopi�s de do/make_checkpoint)
    // un state, pour sauver l'�tat courant
    eoState state;
    ...
    // SELECTION
    // TODO cet op�rateur, fait soit un tri de la pop (true), soit un shuffle (false), id�alement, on ne voudrait ni l'un ni l'autre, car on parcours tout, peu importe l'ordre
    ...
    if ( toursize == 1 ) {
    // L'article indique qu'il n'y a pas de s�lection, on aurait alors �a :
      p_selectone = (eoSelectOne<daex::Decomposition> *) ( new eoSequentialSelect<daex::Decomposition> ( true ) );
    }
    else {
    /// MAIS le code utilise un tournoi d�terministe, on a donc �a :
...
//    daex::MutationDelOneAtom delatom;
    ...
    // (maxtry � 0 pour essayer tous les atomes)
    ...
    // JACK : L'article indique qu'on fait un remplacement en tournoi d�terministe et qu'il n'y a pas d'�listisme, on aurait alors �a :
    //eoSSGADetTournamentReplacement<daex::Decomposition> replace(5);
    
    // MODIFS MS START
    // plus de param�trage du remplacement
    eoReplacement<daex::Decomposition> * pt_replace;
    // MAIS le code utilise un remplacement int�gral des parents par le meilleur des parents+enfants, avec �litisme faible, on a donc 
    // Note MS: le commentaire �tait incoh�rent entre Plus et Comma ???
   ...
   //PEO ADD
    ...
    peoPopEval<daex::Decomposition> eval(*p_eval);

//Penser à Atom et à un index qui permet de lié le'atom défini dans la stucture de YAHSP
 
//    peo :: init( argc, argv );
// 
//    peoWrapper daeParallel( dae, pop);
// 
//    
//    eval.setOwner(daeparallel); 
// 
// 
// if (getNodeRank()==1)
//     {
