...
def parse( files, markers = ["#nodes:"] ):
    ...
    for filename in files:
        ...
        if not result_found:
            print "Warning, %s does not contains one of the markers %s" % ( filename, markers )

    return nodes


def parse_by_instances( files, instances = range(31) ):
...
def histo( data, file_match='', range_max=0, interactive=False ):
    ...
    output = "nodes_distribution_" + file_match.replace('*', '') + ".png"
    ...
    fig = p.figure()
    fig.suptitle( "Distribution of the number of nodes at init" )

    ax = fig.add_subplot(111)

#    ax.set_title( "Distribution of the number of nodes used by YAHSP during\nevaluations of random decompositions in the initialization operator\n%s" %  files_common_match( files ) )
...
def boxplot( data, file_match='', interactive=False ):
    ...
    output = "nodes_distributions_" + file_match.replace('*', '') + ".png"
    ...
    fig.suptitle( "Distributions of the number of nodes at init by instances" )
