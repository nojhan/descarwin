...
#include "pack/peoDaePopEval.h"
#include "evaluation/peoDaeYahspEval.h"
...
eoDualFitness<double, std::greater<double> > dummyFuction(const daex::Decomposition  & _eo)
{
  eoDualFitness<double, std::greater<double> > fit;
  
  fit.setValue(18);
  fit.setFeasible(false);
  return (fit);
}
...
//    bindDaeYahsp( pddl );
    ...
    eoPopLoopEval<daex::Decomposition>  eval( eval_maxtime );
    eval( pop, pop );
...
//    daex::MutationDelOneAtom delatom;
    ...
    // peoEvalFunc<daex::Decomposition>  plainEval(eval_counter);  
    
    
     eoDummyEval plainEval; 
     
     //peoEvalFunc<daex::Decomposition> plainEval( dummyFuction);  
     
     //peoDaeYahspEval<daex::Decomposition> peoYahspEval(eval_yahsp);
     
     peoPopEval<daex::Decomposition> pop_eval ( eval_counter);  // pour tester avec un  evaluateur factice mettre à la place  plainEval;
     
    eoEasyEA<daex::Decomposition> dae( checkpoint, eval_counter,  breed, replace ); // ici aussi pour le test factice
